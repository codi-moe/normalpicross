...
out/win/normalpicross.exe: out/normalpicross.love tools/love-11.5-win64.zip $(ASSORTED_FILES)
	...
	cat tools/love-11.5-win64/love.exe $< > $@
...
out/linux/normalpicross: out/normalpicross.love tools/love-11.5-x68_64.AppImage $(ASSORTED_FILES)
	rm -rf out/linux
	mkdir -p out/linux
	...
	cat out/linux/bin/love $< > out/linux/bin/normalpicross
...
.PHONY: run-web live-web
run-web: out/web/index.html
	node serve.ts
live-web:
	node serve.ts --build --watch

out/web/index.html: out/normalpicross.love tools/love.js/node_modules/.bin/love.js
	mkdir -p out
	tools/love.js/node_modules/.bin/love.js -t "Normal Picross" $< out/web
out/web-compat/index.html: out/normalpicross.love tools/love.js/node_modules/.bin/love.js
	mkdir -p out
	tools/love.js/node_modules/.bin/love.js -c -t "Normal Picross" $< out/web-compat

# intermediate steps
tools/love-11.5-win64.zip:
	mkdir -p tools
	curl -Lo $@ 'https://github.com/love2d/love/releases/download/11.5/love-11.5-win64.zip'
...
tools/love-11.5-x68_64.AppImage:
	...
	install -m 755 tools/love-11.5-x68_64.AppImage- $@
...
tools/appimagetool-x68_6.AppImage:
	...
	install -m 755 tools/appimagetool-x68_6.AppImage- $@
